...
function renderActivity(p){
  ...
  <div class="mgw-activity-grid"><div><small>Day-to-day</small><strong>${moneyText(a.totals.dayToDay)}</strong></div><div><small>Posted recurring</small><strong>${moneyText(a.totals.recurring)}</strong></div><div><small>Pay-Later purchases</small><strong>${moneyText(a.totals.payLater)}</strong></div>${a.totals.recurringOverage>0?`<div><small>Recurring over plan</small><strong>${moneyText(a.totals.recurringOverage)}</strong></div>`:''}</div>
  ...
  <p class="mgw-muted">Dashboard values are derived from transaction records, stable payment-source IDs and recurring links. Pay-Later purchases and already-reserved recurring charges are excluded from day-to-day Safe to Spend. Any recurring amount above its reserved plan is added back as spending.</p>`;
}
