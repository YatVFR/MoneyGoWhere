...
(() => {
  ...
  const persist=()=>{const raw=JSON.stringify(db);localStorage.setItem(MGW.key,raw);if(localStorage.getItem(MGW.key)!==raw)throw new Error('Credit data storage verification failed');};
  const closeModalSafe=()=>{const m=document.querySelector('#modal');if(m?.open)try{m.close()}catch(err){window.MGWUATDiagnostics?.capture?.(err,'credit-modal-close')}};
  const refreshCreditUI=()=>{try{renderCredit()}catch(err){window.MGWUATDiagnostics?.capture?.(err,'credit-targeted-refresh')}};
