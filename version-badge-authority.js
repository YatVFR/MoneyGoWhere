...
(()=>{
  ...
  function apply(){
    ...
    badge.title=`Development · App ${RELEASE} · authoritative DEV badge`;
  }
  function observe(){
    apply();
    const target=document.querySelector('.topbar')||document.body;
    if(!target)return;
    new MutationObserver(()=>queueMicrotask(apply)).observe(target,{subtree:true,childList:true,characterData:true});
  }
