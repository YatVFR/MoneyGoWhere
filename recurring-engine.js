...
function sync(database=window.db||{},options={}){
  ...
  const retained=database.recurringItems.filter(x=>x?.sourceCollection==='canonical').map(clone);
  const next=[...retained];
...
window.MGWRecurringEngine=Object.freeze({version:RELEASE,modelVersion:MODEL_VERSION,ensure,sync,active,itemsForMonth,incomeForMonth,commitmentItemsForMonth,installmentItemsForMonth,totalForMonth,addItem,describe,monthKey});
})();