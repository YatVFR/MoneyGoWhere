// MoneyGoWhere v1.5.5-dev.50 — Recurring Bills & Subscriptions
...
function render(){
  ...
  host.innerHTML=`<div class="card-head"><div><span class="section-icon">🧾</span><b>Recurring Bills & Commitments</b></div></div><p class="mgw-muted">Subscriptions, services, insurance, telecom and other bills. Set an optional End Month so a bill stops being counted automatically when it finishes.</p><div class="mgw-bill-toolbar"><button class="primary-btn" data-bill-add>＋ Add Recurring Bill</button><button data-bill-detect>🔎 Find Recurring Charges</button></div><div class="mgw-bill-list">${grouped||'<p class="mgw-empty">No recurring bills configured yet. Add one once and MGW will include it in future cycles automatically.</p>'}</div><div class="mgw-bill-total"><span>Active recurring bills this cycle</span><strong>${money(total)}</strong></div><div id="mgwRecurringCandidates"></div>`;
}
...
function openBill(x={}){
  ...
  f.addEventListener('submit',e=>{e.preventDefault();const o=Object.fromEntries(new FormData(f));if(o.endMonth&&monthIndex(o.endMonth)<monthIndex(o.startMonth)){toast('End month cannot be before start month');return}o.amount=num(o.amount);o.dueDay=o.dueDay===''?'':Math.min(31,Math.max(1,Number(o.dueDay)||1));o.id=x.id||uid();o.active=x.active!==false;o.confidence=x.confidence||'manual';o.seedKey=x.seedKey;if(edit)Object.assign(x,o);else db.recurringBills.push(o);save();document.querySelector('#modal').close();toast(edit?'Recurring bill updated':'Recurring bill added')},{once:true});
...
window.MGWRecurringBills={version:RELEASE,ensure,billsForMonth,totalForMonth,openEditor:openBill};
...
function boot(){if(!ensure())return;installStyles();installUI();render();bind();waitForCommitmentBridge();if(typeof renderAll==='function'&&!renderAll.__mgwRecurringBills){const base=renderAll;const wrapped=function(){base();render();bind()};wrapped.__mgwRecurringBills=true;renderAll=wrapped}}
