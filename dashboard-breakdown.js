// MoneyGoWhere v1.5.5-dev.7 — resilient dashboard spending breakdown
(()=>{
'use strict';
const RELEASE='1.5.5-dev.7';
...
function styles(){
  ...
  s.textContent=`
    ...
    .mgw-spend-detail,.mgw-recurring-detail,.mgw-recurring-group{border:1px solid var(--border,#dbe4e4);border-radius:14px;overflow:hidden}
    .mgw-spend-detail>summary,.mgw-recurring-detail>summary,.mgw-recurring-group>summary{list-style:none;cursor:pointer;display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center}
    .mgw-spend-detail>summary,.mgw-recurring-detail>summary{padding:12px}
    .mgw-recurring-group>summary{padding:10px 12px;background:rgba(18,130,119,.035)}
    .mgw-spend-detail>summary::-webkit-details-marker,.mgw-recurring-detail>summary::-webkit-details-marker,.mgw-recurring-group>summary::-webkit-details-marker{display:none}
    .mgw-spend-main{display:flex;gap:10px;align-items:center}.mgw-spend-copy b,.mgw-spend-copy small{display:block}
    .mgw-spend-copy small,.mgw-spend-summary-amount small,.mgw-line small,.mgw-group-copy small{opacity:.65}
    ...
    .mgw-recurring-groups{display:grid;gap:8px}
    .mgw-group-copy b,.mgw-group-copy small{display:block}.mgw-group-amount{text-align:right;white-space:nowrap}.mgw-group-amount strong,.mgw-group-amount small{display:block}
    .mgw-group-body{padding:0 12px 6px;border-top:1px solid #eef1f1}
...
function recurring(){
  ...
  const body=[...groups].map(([g,list])=>{
    ...
    const lines=list.map(x=>`<div class="mgw-line"><div><b>${esc(x.name||x.merchant||'Recurring bill')}${x.confidence==='review'?'<span class="mgw-review">Review</span>':''}</b><small>${esc([x.category,x.frequency,x.dueDay?`around day ${x.dueDay}`:'',x.active===false?'Paused':''].filter(Boolean).join(' · '))}</small></div><strong>${money(num(x.amount))}</strong></div>`).join('');
    return `<details class="mgw-recurring-group"><summary><div class="mgw-group-copy"><b>${esc(g)}</b><small>${list.length} ${list.length===1?'item':'items'} · ${activeList.length} active</small></div><div class="mgw-group-amount"><strong>${money(subtotal)}</strong><small>tap to expand</small></div></summary><div class="mgw-group-body">${lines}</div></details>`
  }).join('');
